...
#     tracks, res = tracker.get_object_tracks(
#         video_frames, read_from_stub=True, stub_path='stubs/track_stubs.pkl')
#     vehicle_track_id = list(res.values())
...
#     license_number_list = []
#     for vt_id in vehicle_track_id:
#         original_frame, cropped_frame = tracker.get_vehicle_frames(
#             track_id=vt_id, frames=out_video_frames)

#         output_video_path = "output_video/output.mp4"
#         cropped_frame_path = f"saved_images/cropped.jpg"

#         if original_frame is not None and cropped_frame is not None:
#             show_image(original_frame,
#                        title="Original Frame", save=True)
#             show_image(cropped_frame,
#                        title="Cropped Vehicle", save=False)
#         else:
#             print(f"Track ID {vehicle_track_id} not found in any frames.")

#         # Frame that has the cropped licenseImage
#         license_plate = license_Extract(f"saved_images/cropped.jpg")

#         cv2.imwrite(cropped_frame_path, license_plate,
#                     [cv2.IMWRITE_JPEG_QUALITY, 90])

#         # Save the License Number of the car
#         license_number = Ocr_reader(f'saved_images/cropped.jpg')
#         license_number_list += license_number
...
#     return save_output_video, save_output_image, license_number_list
# ------------------------------------------MULTIPLE LICENSE NUMBER DETECTION
